...
int GameBoard::get_food_x() const
{
    return food_.return_x();
}

int GameBoard::get_food_y() const
{
    return food_.return_y();
}

int GameBoard::get_snake_head_x() const
{
    // Using the getHead()-method to return the snake's heads x-location.
    return getHead().return_x();
}

int GameBoard::get_snake_head_y() const
{
    // Using the getHead()-method to return the snake's heads y-location.
    return getHead().return_y();
}

std::deque<Point> GameBoard::return_snake() const
{
    // Returning the snake_ deque of Point's.
    return snake_;
}

bool GameBoard::collision_with_self(std::string prev_direction) const
{
    Point snake_head = getHead();
    snake_head.move(prev_direction);

    for ( auto& snake_point : snake_ )
    {
        if ( snake_point.return_x() == snake_head.return_x() and
             snake_point.return_y() == snake_head.return_y() )
        {
            // collision with itself, returning true
            return true;
        }
    }
    return false;
